...
count_plot <- mussina %>%
  ...
  scale_fill_gradient2("tOPS+", low = "grey10", high = 'blue',
...
pbp16rc %>%
  ...
  group_by(c11, c20) %>%
  summarize(N = n(), mean_run_value = mean(RUNS.VALUE))
# In a 1-0 count, the difference between a second ball or a first strike is about 0.16 expected runs

# B
# The 2-0 count in my analysis offers more expected runs than what Walsh calculated by about 0.05.

# Exercise 2 
# A 
pbp16rc %>% 
  group_by(BAT_LINEUP_ID) %>% 
  summarize(N = n(), pitch_len = mean(nchar(pseq)))
# It appears that all are about 4 pitches on average, 
# but batters that bat fourth tend to have the longest plate appearances.

# B 
pbp16rc %>%
  filter(AWAY_TEAM_ID == 'CHN') %>% 
  group_by(BAT_LINEUP_ID) %>% 
  summarize(N = n(), pitch_len = mean(nchar(pseq)))
...
pbp16rc %>%
  ...
  group_by(BAT_LINEUP_ID) %>% 
  summarize(N = n(), pitch_len = mean(nchar(pseq)))
# The 8th batter for the Cubs averaged 3.87 pitches per plate appearance while
# the 8th batter for the Mariners averaged 3.88 pitches per plate appearance.

# Exercise 3
first_only <- pbp16rc %>% 
  filter(BASE2_RUN_ID=="" & BASE1_RUN_ID!="") %>% 
  filter(grepl("1", PITCH_SEQ_TX)) %>% 
  group_by(BASE1_RUN_ID) %>%
  summarize(N=n()) %>% 
  arrange(desc(N))
first_only
# villj and bettm were the highest

# Exercise 4
# I can't seem to access the data on umpires. 
